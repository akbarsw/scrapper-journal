...
export const FilterSidebar: React.FC<FilterSidebarProps> = ({ isOpen, onClose, onApply }) => {
  ...
  return (
    <>
      ...
      <div className="fixed top-0 right-0 h-full w-[380px] bg-white z-[100] shadow-2xl flex flex-col transform transition-transform duration-300 ease-in-out border-l border-gray-200">
        
        <div className="flex items-center justify-between p-5 border-b border-gray-100 shrink-0">
          <h2 className="text-[17px] font-bold text-gray-900">Filter {activeFiltersCount > 0 ? `(${activeFiltersCount})` : ''}</h2>
          <button onClick={onClose} className="p-1.5 hover:bg-gray-100 rounded-lg text-gray-500 transition-colors border border-gray-200">
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar">
          ...
          <div className="border-b border-gray-100">
            <button onClick={() => setGeneralExpanded(!generalExpanded)} className="w-full flex items-center justify-between p-5 hover:bg-gray-50 transition-colors">
              ...
              <div className="px-5 pb-5 space-y-6">
                ...
                <div className="space-y-3">
                  ...
                  <div className="relative">
                    <select value={lang} onChange={e => setLang(e.target.value)} className="w-full h-10 px-3 text-sm border border-gray-300 rounded-lg focus:outline-none focus:border-blue-500 appearance-none bg-white font-medium">
                ...
                <div className="space-y-3">
                  ...
                  <div className="relative">
                    <select value={limit} onChange={e => setLimit(Number(e.target.value))} className="w-full h-10 px-3 text-sm border border-gray-300 rounded-lg focus:outline-none focus:border-blue-500 appearance-none bg-white font-medium">
                ...
                <div className="pt-2 space-y-4">
                  <div className="flex items-center justify-between">
                    ...
                    <button onClick={() => setOpenAccess(!openAccess)} className={`w-11 h-6 rounded-full transition-colors relative flex items-center px-0.5 ${openAccess ? 'bg-[#3b82f6]' : 'bg-gray-300'}`}>
          ...
          <div className="border-b border-gray-100">
            <button className="w-full flex items-center justify-between p-5 hover:bg-gray-50 transition-colors opacity-60">
        ...
        <div className="p-5 border-t border-gray-200 bg-white flex gap-3 shrink-0">
          <button onClick={handleApply} className="flex-1 bg-[#3b82f6] hover:bg-blue-600 text-white font-bold text-[15px] h-12 rounded-xl transition-colors">
            Apply {activeFiltersCount > 0 ? `${activeFiltersCount} filter${activeFiltersCount > 1 ? 's' : ''}` : ''}
          </button>
          <button onClick={handleReset} className="px-6 border border-gray-300 hover:bg-gray-50 text-gray-900 font-bold text-[15px] h-12 rounded-xl transition-colors">
            Reset
          </button>
        </div>
